...
export const ShortestPathFilter = ({
  startups,
  vcs,
  updateQueryKey,
  startupValue,
  ...
  <Row>
    <Col md={5}>
      <SelectFilter
        ...
        options={startups
          .toJS()
          .map(startup => ({ value: startup, label: startup }))}
        onChange={value =>
          updateQueryKey(STARTUP_QUERY_KEY, value ? value.value : value)
        }
        value={startupValue}
      />
    </Col>
    ...
    <Col md={5}>
      <SelectFilter
        ...
        options={vcs
          ...
          .map(startup => ({ value: startup, label: startup }))}
        onChange={value =>
          updateQueryKey(VC_QUERY_KEY, value ? value.value : value)
        }
        value={vcValue}
      />
    </Col>
